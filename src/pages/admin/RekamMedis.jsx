...
function ModalForm({ mode, data, onClose, onSave }) {
  ...
  try {
    const payload = {
      ...
      pendaftaran_id: form.pendaftaran_id
        ? Number(form.pendaftaran_id)
        : null,
    };
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'rekamMedis') {
        ...
        setRekamMedisData(result.data || result.rekam_medis || result);

      } else if (activeTab === 'antrian') {
    ...
    <AdminLayout title="Rekam Medis">
      ...
      {modalForm && (
        <ModalForm
          ...
          onSave={() => { setModalForm(null); fetchData(); }}
        />
      )}
